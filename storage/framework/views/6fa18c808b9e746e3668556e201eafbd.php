...
<?php $__env->startSection('layout'); ?>
    <main id="main-peta" class="container w-full space-y-1 text-gray-600">
        <div class="page-title text-center">
            <h2 class="text-3xl font-bold text-bold my-0 pt-6 pb-2">Peta <?php echo e(ucwords(setting('sebutan_desa'))); ?> <?php echo e(ucwords(identitas('nama_desa'))); ?></h2>
            <a href="<?php echo e(ci_route('')); ?>" class="inline-block">Kembali ke Beranda</a>
        </div>
        <div class="row">
            <div class="col-md-12">
                <?php echo $__env->make('theme::commons.loading', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                <div id="map">
    ...
    <script>
        (function() {
            ...
            window.onload = function() {
                const _url = `<?php echo e(ci_route('internal_api.peta')); ?>`
                ...
                const generatePopupDesa = function(data) {
                    let _listLink = [],
                        _elmPopup
                    const _link = '<?php echo e(ci_route('statistik_web.chart_gis_desa')); ?>'
                    _elmPopup = document.getElementById('isi_popup')
                    _elmPopup.querySelector('#content').querySelector('#firstHeading').innerHTML = `Wilayah <?php echo e(ucwords(setting('sebutan_desa'))); ?> ${data.desa.nama_desa}`
                    const _title = `Statistik Penduduk <?php echo e(ucwords(setting('sebutan_desa'))); ?> ${capitalizeFirstCharacterOfEachWord(data.desa.nama_desa)}`
                    // statistik penduduk
              ...
              ${_listLink.join('')}
                    ...
                    if (data.pengaturan.includes('Aparatur Desa')) {
                        _elmPopup.querySelector('#content').querySelector('#bodyContent').innerHTML +=
                            `<p><a href="<?php echo e(ci_route('load_aparatur_desa')); ?>" class="btn btn-social bg-navy btn-sm btn-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" data-title="<?php echo e(ucwords(setting('sebutan_pemerintah_desa'))); ?>" data-remote="false" data-toggle="modal" data-target="#modalKecil"><i class="fa fa-user"></i>&nbsp;&nbsp;<?php echo e(ucwords(setting('sebutan_pemerintah_desa'))); ?>&nbsp;&nbsp;</a></p>`
                    }
                }
                const generatePopupDusun = function(data) {
                    const _elmPopup = document.getElementById('isi_popup_dusun')
                    const _link = '<?php echo e(ci_route('statistik_web.chart_gis_dusun')); ?>'
                    const _title = `<?php echo e(ucwords(setting('sebutan_desa'))); ?> ${capitalizeFirstCharacterOfEachWord(data.desa.nama_desa)}`
                    const _wilayah = {
                        level: 1,
                        key: 'dusun',
                        sebutan: "<?php echo e(ucwords(setting('sebutan_kepala_dusun'))); ?>",
                ...
                const generatePopupRw = function(data) {
                    ...
                    const _link = '<?php echo e(ci_route('statistik_web.chart_gis_rw')); ?>'
                    const _title = `<?php echo e(ucwords(setting('sebutan_dusun'))); ?>`
                    const _wilayah = {
                        level: 2,
                        key: 'rw',
                        sebutan: "RW",
                ...
                const generatePopupRt = function(data) {
                    ...
                    const _link = '<?php echo e(ci_route('statistik_web.chart_gis_rt')); ?>'
                    const _title = `<?php echo e(ucwords(setting('sebutan_dusun'))); ?>`
                    const _wilayah = {
                        level: 3,
                        key: 'rt',
              ...
              <div class="card card-body">
                ...
                ${_listLink.join('')}
                        ...
                        if (pengaturan.includes('Aparatur Desa')) {
                            _contentHTML +=
                                `<p><a href="<?php echo e(ci_route('load_aparatur_wilayah')); ?>/${gis[_key]['id_kepala']}/${_wilayah['level']}" class="btn btn-social bg-navy btn-sm btn-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" data-title="<?php echo e(ucwords(setting('sebutan_kepala_dusun')) . ' ' . $dusun['dusun']); ?>" data-remote="false" data-toggle="modal" data-target="#modalKecil"><i class="fa fa-user"></i>&nbsp;&nbsp;${_wilayah['sebutan']}&nbsp;&nbsp;</a></p>`
                        }
                        _elemenHTML = `
            ...
            <div id="content">
              <h5 id="firstHeading" class="firstHeading">Wilayah ${_newTitle}</h5>
                ...
                const generatePeta = function(data) {
                    ...
                    let LOKASI_SIMBOL_LOKASI = '<?php echo e(base_url(LOKASI_SIMBOL_LOKASI)); ?>';
                    let favico_desa = '<?php echo e(favico_desa()); ?>';
                    let LOKASI_FOTO_AREA = '<?php echo e(base_url(LOKASI_FOTO_AREA)); ?>';
                    let LOKASI_FOTO_GARIS = '<?php echo e(base_url(LOKASI_FOTO_GARIS)); ?>';
                    let LOKASI_FOTO_LOKASI = '<?php echo e(base_url(LOKASI_FOTO_LOKASI)); ?>';
                    let LOKASI_GALERI = '<?php echo e(base_url(LOKASI_GALERI)); ?>';
                    let info_pembangunan = '<?php echo e(ci_route('pembangunan')); ?>';
                    let all_persil = JSON.stringify(data.persil)
                    let TAMPIL_LUAS = '<?php echo e(setting('tampil_luas_peta')); ?>';
                    let PENGATURAN_WILAYAH = '<?php echo SebutanDesa(setting('default_tampil_peta_wilayah')) ?: []; ?>';
                    let PENGATURAN_INFRASTRUKTUR = '<?php echo SebutanDesa(setting('default_tampil_peta_infrastruktur')) ?: []; ?>';
                    let WILAYAH_INFRASTRUKTUR = PENGATURAN_WILAYAH.concat(PENGATURAN_INFRASTRUKTUR);

                    //OVERLAY WILAYAH DESA
                    if (data.desa['path']) {
                        set_marker_desa_content(marker_desa, data.desa, "<?php echo e(ucwords(setting('sebutan_desa'))); ?> ${data.desa['nama_desa']}", "<?php echo e(favico_desa()); ?>", '#isi_popup');
                    }

                    //OVERLAY WILAYAH DUSUN
                    if (data.dusun_gis) {
                        set_marker_multi_content(marker_dusun, JSON.stringify(data.dusun_gis), '<?php echo e(ucwords(setting('sebutan_dusun'))); ?>', 'dusun', '#isi_popup_dusun_', '<?php echo e(favico_desa()); ?>');
                    }

                    //OVERLAY WILAYAH RW
                    if (data.rw_gis) {
                        set_marker_content(marker_rw, JSON.stringify(data.rw_gis), 'RW', 'rw', '#isi_popup_rw_', '<?php echo e(favico_desa()); ?>');
                    }

                    //OVERLAY WILAYAH RT
                    if (data.rt_gis) {
                        set_marker_content(marker_rt, JSON.stringify(data.rt_gis), 'RT', 'rt', '#isi_popup_rt_', '<?php echo e(favico_desa()); ?>');
                    }
                    ...
                    let baseLayers = getBaseLayers(mymap, "<?php echo e(setting('mapbox_key')); ?>", "<?php echo e(setting('jenis_peta')); ?>");
                    ...
                    mymap.on('overlayadd', function(eventLayer) {
                        if (eventLayer.name === 'Peta Wilayah Desa') {
                            setlegendPetaDesa(legenda_desa, mymap, data.desa, '<?php echo e(ucwords(setting('sebutan_desa'))); ?>', data.desa['nama_desa']);
                        }
                        if (eventLayer.name === 'Peta Wilayah Dusun') {
                            setlegendPeta(legenda_dusun, mymap, JSON.stringify(data.dusun_gis), '<?php echo e(ucwords(setting('sebutan_dusun'))); ?>', 'dusun', '', '');
                        }
                        if (eventLayer.name === 'Peta Wilayah RW') {
                            setlegendPeta(legenda_rw, mymap, JSON.stringify(data.rw_gis), 'RW', 'rw', '<?php echo e(ucwords(setting('sebutan_dusun'))); ?>');
                        }
...
<?php echo $__env->make('theme::template', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/akmalfadli/Developer/desa-digital/wiradesa//storage/app/themes/perwira/resources/views/partials/peta/index.blade.php ENDPATH**/ ?>